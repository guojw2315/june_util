...
var _refreshToken = function refreshToken(instance, path, baseURL) {
  ...
  return instance.post(path + ("?refreshToken=" + (0, _auth.getRefreshToken)()), {
...
var _setHeaderToken = function setHeaderToken(instance, isNeedToken, options) {
  ...
  var refreshToken = isNeedToken ? (0, _auth.getRefreshToken)() : null;
  var accessToken = isNeedToken ? (0, _auth.getToken)() : null;

  if (isNeedToken) {
    // api 请求需要携带 access_token
    var tokenLose = (0, _auth.getTokenState)();

    if (!refreshToken && !tokenLose) {
      console.log('不存在 access_token 则跳转回登录页');
      (0, _auth.setTokenState)(true);
...
var setPosHeaders = function setPosHeaders(instance, key) {
  if (key === void 0) {
    key = 'x-shj-request-dept-pos-id';
  }

  instance.defaults.headers.common[key] = (0, _auth.getPosId)();
};

exports.setPosHeaders = setPosHeaders;
